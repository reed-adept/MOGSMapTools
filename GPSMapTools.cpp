...
#include "Aria.h"
#include "ArServerClasses.h" // from ARNL
#include "GPSMapTools.h"
#include <assert.h>

AREXPORT GPSMapTools::GPSMapTools(ArGPS *gps, ArRobot* robot,
  ArServerHandlerCommands *commandServer, ArMap *map, ArServerHandlerMap *ms,
  ArServerHandlerLocalization *locServer, ArServerHandlerPopup *popupServer
) :
  myGPS(gps),
  myMap(map),
  myRobot(robot),
  ...
  mySetMapOriginLLAFromGPSCB(this, &GPSMapTools::setMapOriginLLAFromGPS),
  ...
  myResetMapOriginHereCB(this, &GPSMapTools::resetMapOriginHere),
  myMapServer(ms),
  myLocalizationServer(locServer)
{
 ...
 commandServer->addCommand("Map:setMapOriginLLAFromGPS", "Change the map origin's georeference coordinates to the current GPS position to the map. Only use this command if the robot is at the map center point (origin of map). Use this to add a georeference point to a map generated by laser scanning and processing.", &mySetMapOriginLLAFromGPSCB);
  commandServer->addCommand("Map:resetMapOriginHere", "Use the current GPS position and robot's map position to create a new georeferenced map origin. Map objects and data is moved accordingly. Use this command if the robot position is correct with respect to map objects and has a good GPS position that can be a good map georeference point.", &myResetMapOriginHereCB);
...
void GPSMapTools::resetRobotPose()
{
  if(myRobot)
  {
    ...
    myRobot->unlock();
    ArUtil::sleep(200);
    if(myLocalizationServer)
    {
      myLocalizationServer->localizeToPose(0,0,0);
    }
    else
    {
      myRobot->lock();
...
void GPSMapTools::setMapOriginLLAFromGPS()
{
  if(!checkGPS("setting map origin from GPS")) return;
  if(!checkMap("setting map origin from GPS")) return;
  resetRobotPose();
...
void GPSMapTools::resetMapOriginHere()
{
  if(!checkGPS("resetMapOriginHere")) return;
  if(!checkMap("resetMapOriginHere")) return;
  if(myMap->hasOriginLatLongAlt())
  {
    ArLog::log(ArLog::Terse, "GPSMapTools: setting map origin: Warning: Map already has an origin point, it will be replaced by current position.");
  }
  myRobot->lock();
  ArPose robotpose(myRobot->getPose());
  myRobot->unlock();
  resetRobotPose();
  myOrigin = myGPS->getLLA();
  resetMapCoords(myOrigin);
  myMap->lock();
  myMap->setOriginLatLongAlt(true, myGPS->getAsPose(), myGPS->getAltitude());
  myMap->offsetMapContents(ArPose(-1 * robotpose.getX(), -1 * robotpose.getY()));
  myMap->mapChanged();
  myMap->writeFile(myMap->getFileName(), true);
  myMap->unlock();
  reloadMapFile();
